...
void Camera::setViewByMouse(int x,int y)
{
	...
	/**< 得到鼠标移动方向 */
	//cout << lastmousePos.x << endl;

	angleY = (float)((lastmousePos.x - mousePos.x)) / 100.0f;
	angleZ = (float)((lastmousePos.y - mousePos.y)) / 250.0f;
...
void Camera::moveCamera(float speed)
{
	...
	vector = vector.normalize();         /**< 单位化 */
										 ...
										 /** 更新摄像机 */
	m_Position.y += vector.x * speed;    /**< 根据速度更新位置 */
	//m_Position.z += vector.z * speed;
	m_View.y += vector.x * speed;        /**< 根据速度更新方向 */
	//m_View.z += vector.z * speed;
}

/** 前后移动摄像机 */
void Camera::goCamera(float speed)
{
	/** 计算方向向量 */
	Vector3 vector = m_View - m_Position;
	vector = vector.normalize();         /**< 单位化 */
...
void Camera::setLook()
{
	...
	//cout << "upvector " << m_UpVector.x << " " << m_UpVector.y << " " << m_UpVector.z << endl;
